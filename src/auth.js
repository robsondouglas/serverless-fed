...
export const signIn = (usr, pwd, persist) => new Promise((resolve, reject) => {
    ...
    _user.authenticateUser(details, {
        onSuccess: ({idToken, refreshToken, accessToken}) => resolve({name:idToken.payload.name, user:idToken.payload.email, refreshToken:refreshToken.token, accessToken:accessToken.jwtToken}),
        onFailure: reject,
